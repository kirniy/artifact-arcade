...
class CaricatureStyle(Enum):
    ...
    PHOTOBOOTH_PROJECT_X = "photobooth_project_x"
...
JARA_VARIATIONS = [
    """ЖАРА — IDENTITY-LOCKED 2D POOL PARTY POSTER (VERTICAL 9:16)

Create one single centered full-color 9:16 poster. No grid, no photo strip, no separate frames.

NON-NEGOTIABLE IDENTITY LOCK — SPEND THE MOST DETAIL ON THE FACES:
- Input image 1 is the real booth photo and the ONLY source of people. Count the people before drawing; include every person exactly once and add nobody.
- Use the source photo as a fixed underdrawing. Rotoscope the existing people in place: keep their original left/center/right order, relative coordinates, scale, head angles, arm gestures, overlaps, and camera perspective. Replace the environment behind them; do not re-stage the group as newly invented pool-party models.
- This is an identity-preserving redraw, not a character redesign. Each guest must be recognizable immediately without seeing the source beside it.
- For EACH face separately, preserve the measured facial geometry: head silhouette, forehead height, hairline, inter-eye distance, eye shape and tilt, eyebrow thickness and arc, nose bridge/length/tip/nostril width, cheek volume, lip contour and width, jaw angle, chin shape, ears, skin tone, age cues, and any facial hair.
- Preserve the exact camera angle, gaze, expression, pose, height relationships, body proportions, hairstyle, hair texture/color, and distinctive asymmetries.
- Trace hair from the source instead of inventing a fashionable substitute: curls remain curls, straight hair remains straight, the exact fringe/hairline remains, and hair volume and parting do not change.
- Preserve outfit identity exactly: garment type, silhouette, color, neckline, sleeves, accessories, and visible prints/logos/lettering. Do not invent pool clothing or sunglasses on a face.
- Do not homogenize the group into one house style: each person must retain their own different eyes, nose, jaw, hair and expression. No generic handsome models, beauty filter, face averaging, face swapping, de-aging, plastic skin, or caricature exaggeration.
- Render them as refined hand-drawn 2D editorial portraits with precise facial linework, restrained cel shading and subtle print grain. Faces are anatomically accurate; the illustration treatment must not weaken likeness.
...
Edit the attached photograph in place as premium stylized 3D animation with tactile comic-print finishing. Preserve every real face, body proportion, pose, group relationship, and the recognizable real room behind them. Replace each guest's clothes from the neck down with a fitted athletic light coral-red-and-sky-blue suit made from textured technical fabric: black raised web lattice over the red torso and sleeves, pale sky-blue side panels and legs, subtle dark piping, integrated gloves, and a large clearly visible dark spider silhouette centered on the light chest panel; no letters. Every face, hairline, hairstyle, ears and expression must remain fully visible and uncovered; no mask, hood, helmet, eyewear or face paint. Build faces, bodies and environment as genuinely dimensional CGI with modeled volume, perspective depth, cinematic lensing, soft global illumination, volumetric rim light, believable material response and subtle depth of field; this must read as a frame from a high-budget 3D animated feature, not a flat drawing. Over that dimensional render add confident ink contours, halftone shadow fields, offset cyan/red print registration, stepped-motion echoes, paper grain, and sharp graphic light shapes. Use a bright scarlet, cobalt, warm ivory and limited black palette with readable midtones. Integrate the complete Image 2 SPIDERVERSE emblem natively within the upper scene, not in a separate header or white strip. Do not invent franchise characters, studio names, other logos, or any extra readable text. Keep faces out of the lowest 13% for the app footer."""
]

SPIDERVERSE_SQUARE_VARIATIONS = [
    """DIMENSIONAL GRAPHIC-NOVEL MULTIVERSE — SQUARE 1:1
Rebuild the exact source guests and actual venue as a genuinely dimensional high-budget 3D animated still with modeled facial volume, natural adult anatomy, cinematic perspective, global illumination, volumetric rim light, material depth and subtle depth of field, while preserving precise identity. Replace clothes from the neck down with fitted light coral-red-and-sky-blue technical-fabric athletic suits carrying a raised black web lattice, pale blue side panels, fine piping and integrated gloves, but a large clearly visible dark spider silhouette centered on the light chest panel; no letters. Keep every real face, hairstyle and ear completely uncovered: no masks, hoods, helmets, eyewear or face paint. Finish the 3D frame with graphic ink contours, halftone texture, chromatic print separation and layered parallax. Bright scarlet, cobalt and warm ivory dominate; keep shadows readable. Integrate the complete Image 2 SPIDERVERSE emblem natively within the upper scene, not in a separate header or white strip. No franchise character, studio mark, replacement location, fake lettering or additional text."""
]
    ...
    async def generate_caricature(
        ...
        try:
            ...
            if style in {CaricatureStyle.PHOTOBOOTH_PROJECT_X, CaricatureStyle.PHOTOBOOTH_PROJECT_X_SQUARE}:
                ...
                image_data = await generate_project_x(
                    self._client, reference_photo, extra_reference_images,
                    square=style == CaricatureStyle.PHOTOBOOTH_PROJECT_X_SQUARE,
...
- COMPOSITION OVERRIDE: reframe as a close waist-up group portrait. Preserve identity and relative group order but crop surplus ceiling/wall; do not preserve the source's empty headroom. Guests fill at least 70% of image width and heads begin within the upper 25%. No large empty area above them. Keep all actual guests, but never duplicate a person or turn a face-reference crop into an additional guest
- WARDROBE OVERRIDE: replace every guest's original clothes from the neck down with a fitted light coral-red-and-sky-blue technical-fabric athletic suit. Use a raised black web lattice over red torso/sleeves, pale sky-blue side panels and legs, subtle dark piping and integrated gloves. A large clearly visible dark spider silhouette centered on a pale coral chest panel is mandatory; no letters
...
- Image 2 is the exact canonical SPIDERVERSE event emblem. Render its complete geometry and word natively near the top of the scene, around 50-65% image width. Integrate it with the room and lighting, not a separate banner, white strip or pasted header; never cover faces
...
- Image 2 is the canonical silver VNVNC chain pendant, used only as the app-composited hero asset. Do not put pendants, necklaces, chains, medallions, badges, or VNVNC jewelry on any person
...
- Do not model-render any VNVNC lettering. No ВСЕ СВОИ title, caption, paragraph, prompt text, date, weekday, time, address, slogans, labels, signs, garment lettering, pseudo-text or extra logos anywhere
...
- High quality artistic illustration, NOT pixel art, NOT photorealistic"""
  ...
  * You must render one complete supplied SPIDERVERSE event emblem within the scene; no later app logo overlay will be added
  * Preserve any physical emblem/sign already on the real venue wall. That existing background sign is allowed; do not invent additional signs or badges
  ...
  * Do not put any pendant, necklace, chain, medallion, badge, or VNVNC jewelry on any person
...
"""
            ...
            if is_spiderverse_style:
                image_style = "High-budget genuinely dimensional 3D animated-feature frame edited directly from the source photo; modeled facial volume, cinematic perspective and lensing, soft global illumination, volumetric rim light, believable materials and subtle depth of field; exact real uncovered faces and group preserved; every guest wears a fitted light coral-red-and-sky-blue technical-fabric athletic suit with raised black web lattice, pale blue side panels, fine piping and integrated gloves from the neck down; no masks, hoods, helmets, face paint, letters; recognizable real venue preserved; dimensional CGI under bold ink contours, offset cyan-red print registration, halftone fields, stepped-motion echoes, paper grain and layered parallax; complete reference event emblem integrated natively into the upper scene, no separate header; never a flat drawing; no franchise character, studio mark, extra text, dark void or generic replacement scene"
