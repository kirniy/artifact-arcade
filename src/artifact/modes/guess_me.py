...
# =============================================================================
# AI PROMPTS - Conviction/Diagnostic Style
# Combines forensic investigation with pseudo-medical terminology
# =============================================================================

import random as _random

# Multiple analysis approaches for variety
ANALYSIS_APPROACHES = [
    # APPROACH 1: FORENSIC ANALYST - Evidence-based personality profiling
    """Ты форензик-аналитик личности в аркадном автомате VNVNC. Улики не лгут. Составляешь досье на человека по фото и ответам.
...
1. ВНЕШНОСТЬ с фото: выражение глаз, поза, одежда, выражение лица
2. ОТВЕТЫ НА ВОПРОСЫ: каждый ответ это улика!""",

    # APPROACH 2: PERSONALITY DIAGNOSTICIAN - Medical/clinical absurdity
    """Ты диагност личности в аркадном автомате VNVNC. Проводишь вскрытие характера. Псевдомедицинская терминология, клинический тон.
...
=== ОБЯЗАТЕЛЬНО АНАЛИЗИРУЙ ===
1. ВНЕШНОСТЬ с фото как симптомы: глаза, поза, одежда, выражение
2. ОТВЕТЫ НА ВОПРОСЫ как данные профиля!""",

    # APPROACH 3: TRIBUNAL JUDGE - Conviction style
    """Ты судья невидимого трибунала личности в аркадном автомате VNVNC. Выносишь вердикт: кто этот человек на самом деле.
...
=== ОБЯЗАТЕЛЬНО АНАЛИЗИРУЙ ===
1. ВНЕШНОСТЬ с фото как доказательства: глаза, поза, одежда
2. ОТВЕТЫ НА ВОПРОСЫ как показания!""",
]

ANALYSIS_PROMPT_TEMPLATE = """{approach}
...
FINAL_GUESS_APPROACHES = [
    # APPROACH 1: FORENSIC VERDICT
    """ДОСЬЕ ЗАКРЫТО. Вердикт по делу.

На основании анализа ВНЕШНОСТИ (лицо, глаза, поза, одежда на фото) и ПОКАЗАНИЙ (ответы на вопросы) установлено:""",

    # APPROACH 2: DIAGNOSTIC CONCLUSION
    """СКАНИРОВАНИЕ ЗАВЕРШЕНО. Диагноз поставлен.

На основании симптомов ВНЕШНОСТИ (выражение лица, поза, стиль с фото) и результатов ТЕСТА (ответы на вопросы) выявлено:""",

    # APPROACH 3: TRIBUNAL CONVICTION
    """ТРИБУНАЛ ВЫНЕС РЕШЕНИЕ.

Рассмотрев улики ВНЕШНОСТИ (черты лица, выражение глаз, одежда) и ПРИЗНАНИЯ (ответы на вопросы) суд постановил:""",
]

FINAL_GUESS_PROMPT_TEMPLATE = """Ты выносишь финальный вердикт в аркадном автомате VNVNC.
...
2. Как ОТВЕТЫ на вопросы подтвердили или опровергли первое впечатление
3. Финальный вердикт: кто этот человек
...
def get_analysis_prompt(profile: str) -> str:
    """Get a randomly selected analysis prompt."""
    approach = _random.choice(ANALYSIS_APPROACHES)
    return ANALYSIS_PROMPT_TEMPLATE.format(approach=approach, profile=profile)

def get_final_guess_prompt(base_profile: str, extra_profile: str) -> str:
    """Get a randomly selected final guess prompt."""
    approach = _random.choice(FINAL_GUESS_APPROACHES)
    return FINAL_GUESS_PROMPT_TEMPLATE.format(
...
ANALYSIS_PROMPT = ANALYSIS_PROMPT_TEMPLATE.format(
    approach=ANALYSIS_APPROACHES[0],
    profile="{profile}"
)
FINAL_GUESS_PROMPT = FINAL_GUESS_PROMPT_TEMPLATE.format(
    base_profile="{base_profile}",
    extra_profile="{extra_profile}",
    approach=FINAL_GUESS_APPROACHES[0]
)
...
class GuessMeMode(BaseMode):
    ...
    async def _run_analysis(self) -> None:
        ...
        try:
            ...
            prompt = get_analysis_prompt(profile)
    ...
    async def _run_final_analysis(self) -> None:
        ...
        try:
            ...
            prompt = get_final_guess_prompt(base_profile, extra_profile)
