...
=== НАЗВАНИЕ ТРЕКА ===
ВАЖНО: Название трека ОБЯЗАТЕЛЬНО должно быть на РУССКОМ языке!
...
- Краш в Душе
- Огонь и Лёд
НЕ ИСПОЛЬЗУЙ английские названия типа "Night Vibes" или "Moon Code"!
...
{
  "title": "Название трека НА РУССКОМ (креативное, запоминающееся)",
  "artist": "ARTIFACT AI",
...
class LyricsGenerator:
    ...
    async def generate_lyrics(
        ...
        try:
            ...
            # Single unified call - Gemini 3 Flash is multimodal
            logger.info(f"Generating lyrics (Gemini 3 Flash, has_photo={photo_data is not None})")
            response = await self._client.generate_text(
                ...
                temperature=0.95,
                max_tokens=2048,
                image_data=photo_data,  # Optional - works with or without
            )
