...
QUIZ_QUESTIONS: List[Tuple[str, List[str], int]] = [
    ...
    ("Где родилась INSTASAMKA?", ["Казахстан", "Россия", "Украина", "Беларусь"], 0),
    ("Настоящее имя INSTASAMKA?", ["Дарья Зотеева", "Мария Иванова", "Анна Петрова", "Ольга Сидорова"], 0),
    ("Альбом 'Красота и уродство' чей?", ["Oxxxymiron", "Скриптонит", "Хаски", "Face"], 0),
    ("Год выхода 'Красота и уродство'?", ["2022", "2021", "2023", "2020"], 0),
    ...
    ("Где сейчас MORGENSHTERN?", ["За границей", "В России", "В тюрьме", "Неизвестно"], 0),
    ("Год ухода MORGENSHTERN из России?", ["2022", "2021", "2023", "2020"], 1),
    ("Трек 'Дико тусим' от?", ["Big Baby Tape", "OG Buda", "MAYOT", "Платина"], 0),
    ("Кто такой Big Baby Tape?", ["Рэпер из Москвы", "DJ", "Продюсер", "Блогер"], 0),
    ("OG Buda настоящее имя?", ["Роман Милованов", "Алексей Петров", "Дмитрий Сидоров", "Иван Кузнецов"], 0),
    ...
    ("Скриптонит город?", ["Павлодар", "Москва", "Алматы", "Астана"], 0),
    ...
    ("Платина рэпер откуда?", ["Москва", "Питер", "Казань", "Екатеринбург"], 0),
    ...
    ("Настоящее имя ANNA ASTI?", ["Анна Дзюба", "Анна Астафьева", "Анна Иванова", "Анна Петрова"], 0),
    ...
    ("Трек 'Любимка' год выхода?", ["2020", "2019", "2021", "2022"], 0),
    ("JONY настоящее имя?", ["Джахид Гусейнли", "Иван Петров", "Алексей Сидоров", "Дмитрий Кузнецов"], 0),
    ("JONY откуда?", ["Азербайджан", "Россия", "Казахстан", "Армения"], 0),
    ("Хит JONY 2020?", ["Комета", "Звезда", "Луна", "Солнце"], 0),
    ...
    ("Что значит 'ауф'?", ["Восхищение", "Злость", "Грусть", "Страх"], 0),
    ("'Гигачад' это?", ["Идеальный мужчина мем", "Рэпер", "Блогер", "Актёр"], 0),
    ("Что такое 'NPC'?", ["Скучный человек", "Крутой парень", "Умный", "Спортсмен"], 0),
    ("Мем 'ризз' означает?", ["Обаяние", "Злость", "Грусть", "Страх"], 0),
    ("Что такое 'слэй'?", ["Круто выглядеть", "Грустить", "Спать", "Есть"], 0),
    ("'Делулу' это?", ["В своих фантазиях", "Умный", "Спортивный", "Богатый"], 0),
    ("Мем 'Skibidi' создал?", ["DaFuq!?Boom!", "PewDiePie", "MrBeast", "А4"], 0),
    ("Что значит 'no cap'?", ["Без шуток", "Шутка", "Вопрос", "Ответ"], 0),
    ...
    ("Что значит 'база'?", ["Правда", "Ложь", "Вопрос", "Ответ"], 0),
    ...
    ("Скандал с Лерчек в 2023?", ["Налоговый", "Развод", "Авария", "Болезнь"], 0),
    ...
    # ===========================================================================
    # RUSSIAN RAP BEEF & DRAMA 2020-2025
    # ===========================================================================
    ("Бифы MORGENSHTERN с кем были?", ["Со многими", "Ни с кем", "Только Oxxxymiron", "Только Face"], 0),
    ("Скандал INSTASAMKA с Ивлеевой?", ["Был", "Не было", "Неизвестно", "Слухи"], 0),
    ("Oxxxymiron vs Гнойный когда?", ["2017", "2018", "2019", "2020"], 0),
    ("Versus Battle закрылся когда?", ["2020", "2019", "2021", "Работает"], 3),
    ("Кто выиграл Oxxxymiron vs Гнойный?", ["Гнойный", "Oxxxymiron", "Ничья", "Отменили"], 0),
    ("Слава КПСС это?", ["Гнойный", "Рэпер", "Блогер", "Всё верно"], 3),
    ("Face ушёл из рэпа?", ["На время", "Навсегда", "Не уходил", "Неизвестно"], 0),
    ...
    # ===========================================================================
    # YOUTUBE RUSSIA 2023-2025
    # ===========================================================================
    ("А4 настоящее имя?", ["Влад Бумага", "Алексей Петров", "Дмитрий Иванов", "Сергей Сидоров"], 0),
    ("А4 откуда?", ["Беларусь", "Россия", "Украина", "Казахстан"], 0),
    ("Сколько подписчиков у А4?", ["50М плюс", "30М", "20М", "10М"], 0),
    ...
    ("Дудь уехал из России?", ["Да", "Нет", "Неизвестно", "Вернулся"], 0),
    ("Кто такой Артур Бабич?", ["Тиктокер", "Рэпер", "DJ", "Актёр"], 0),
    ("Аня Покров это?", ["Тиктокерша", "Рэперша", "DJ", "Актриса"], 0),
    ("Милохин и Медведева вместе?", ["Были", "Не были", "Вместе сейчас", "Неизвестно"], 0),

    # ===========================================================================
    # SLANG & EXPRESSIONS 2023-2025
    # ===========================================================================
    ("Что значит 'пушка'?", ["Круто", "Плохо", "Грустно", "Страшно"], 0),
    ("'Жиза' это?", ["Жизненно", "Грустно", "Весело", "Страшно"], 0),
    ("Что такое 'имба'?", ["Очень сильный", "Слабый", "Средний", "Плохой"], 0),
    ("'Кек' означает?", ["Смех", "Грусть", "Злость", "Страх"], 0),
    ("Что такое 'олды'?", ["Ветераны темы", "Новички", "Дети", "Старики"], 0),
    ("'Сорян' это?", ["Извини", "Привет", "Пока", "Спасибо"], 0),
    ("Что значит 'рили'?", ["Реально", "Нереально", "Может быть", "Никогда"], 0),
    ("'Изи катка' означает?", ["Лёгкая победа", "Сложная игра", "Ничья", "Поражение"], 0),
    ("Что такое 'ливнуть'?", ["Уйти", "Прийти", "Остаться", "Подождать"], 0),
    ("'Чекать' значит?", ["Проверять", "Игнорить", "Удалять", "Добавлять"], 0),
