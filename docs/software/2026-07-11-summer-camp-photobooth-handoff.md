...
- completed photo: hard-cut cycle `ФОТО` / `НА ЧЕКЕ`; `ГОТОВО!` remains on the LCD
- QR view: `QR`
...
The old production brightness `128` produced heavy red noise. Reducing it to `64` helped but occasional moving red dots remained during full labels. `HardwareConfig.ws2812b_brightness=32` plus Summer Camp ticker color `(0,255,48)` eliminated the noise onsite. Other display styling remains unchanged. Brightness `32`, GRB order, static state rendering, compact labels, and no ticker overlays are the baseline for all photobooth themes.
