#!/bin/bash
# Activate today's ВСЕ СВОИ photobooth package.
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Activates the ВСЕ СВОИ photobooth theme."
            exit 0
            ;;
...
set_env PHOTOBOOTH_THEME vse-svoi
set_env PHOTOBOOTH_MENU_MODES vse_svoi
...
PYCACHE_DIR="${ARTIFACT_PYCACHE_DIR:-/tmp/artifact-activate-vse-svoi-pycache}"
rm -rf "${PYCACHE_DIR}"
mkdir -p "${PYCACHE_DIR}"
...
echo "ВСЕ СВОИ photobooth env is active in ${ENV_FILE}"
if [ "${RESTART}" = "1" ]; then
