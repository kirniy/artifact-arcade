#!/bin/bash
# Activate the evergreen Boiling Room weekend photobooth package.
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Activates the evergreen Boiling Room photobooth theme."
            exit 0
            ;;
...
set_env PHOTOBOOTH_THEME boilingroom
set_env PHOTOBOOTH_MENU_MODES boilingroom
...
set_env ARTIFACT_AUTO_ACTIVATE_BOILINGROOM 1
...
PYCACHE_DIR="${ARTIFACT_PYCACHE_DIR:-/tmp/artifact-activate-boilingroom-pycache}"
rm -rf "${PYCACHE_DIR}"
mkdir -p "${PYCACHE_DIR}"
...
echo "Boiling Room photobooth env is active in ${ENV_FILE}"
if [ "${RESTART}" = "1" ]; then
