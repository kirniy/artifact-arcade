#!/bin/bash
# Activate the SPIDERVERSE photobooth package after required-asset QC.

set -euo pipefail
REPO_DIR="${ARTIFACT_REMOTE_DIR:-/home/kirniy/modular-arcade}"
ENV_FILE="${ARTIFACT_ENV_FILE:-${REPO_DIR}/.env}"
RESTART=0
for arg in "$@"; do
    case "$arg" in
        --restart) RESTART=1 ;;
        -h|--help) echo "Usage: $0 [--restart]"; exit 0 ;;
        *) echo "Unknown option: $arg" >&2; exit 2 ;;
    esac
done
cd "${REPO_DIR}"

# Never mutate the live environment unless the mandatory media package exists.
"${REPO_DIR}/scripts/prepare-spiderverse-idle-video.sh" --check
test -s "${REPO_DIR}/assets/images/spiderverse-emblem.png"

mkdir -p "$(dirname "${ENV_FILE}")"
touch "${ENV_FILE}"
set_env() {
    local key="$1" value="$2"
    ENV_FILE="${ENV_FILE}" ENV_KEY="${key}" ENV_VALUE="${value}" python3 - <<'PY'
import os
from pathlib import Path
p = Path(os.environ["ENV_FILE"])
k, v = os.environ["ENV_KEY"], os.environ["ENV_VALUE"]
lines = p.read_text(encoding="utf-8").splitlines() if p.exists() else []
prefix = f"{k}="
out, found = [], False
for line in lines:
    if not line.startswith(prefix) or line.lstrip().startswith("#"):
        out.append(line)
    else:
        out.append(f"{k}={v}")
        found = True
if not found:
    out.append(f"{k}={v}")
p.write_text("\n".join(out).rstrip() + "\n", encoding="utf-8")
PY
}

delete_env() {
    local key="$1"
    ENV_FILE="${ENV_FILE}" ENV_KEY="${key}" python3 - <<'PY'
import os
from pathlib import Path
p = Path(os.environ["ENV_FILE"])
k = os.environ["ENV_KEY"]
if p.exists():
    lines = p.read_text(encoding="utf-8").splitlines()
    p.write_text(
        "\n".join(line for line in lines if not line.startswith(f"{k}=" )).rstrip() + "\n",
        encoding="utf-8",
    )
PY
}

set_env PHOTOBOOTH_THEME spiderverse
set_env PHOTOBOOTH_MENU_MODES spiderverse
set_env PHOTOBOOTH_AI_ENABLED true
set_env PHOTOBOOTH_CAMERA_SELECTOR_ENABLED auto
set_env PHOTOBOOTH_HDMI_CAPTURE_AI_ENABLED true
set_env PHOTOBOOTH_PRINT_FORTUNES false
set_env ARTIFACT_IMAGE_PROVIDER vertex
set_env ARTIFACT_GEMINI_PROVIDER vertex
set_env GOOGLE_GENAI_USE_VERTEXAI true
set_env GOOGLE_CLOUD_PROJECT project-b3b2422d-d764-493a-95d
set_env GOOGLE_CLOUD_LOCATION global
set_env GEMINI_IMAGE_MODEL gemini-3.1-flash-lite-image
delete_env GEMINI_PROXY
set_env VNVNC_INPROCESS_TV_WALL_ENABLED false
set_env ARTIFACT_SPIDERVERSE_QUEST_ENABLED true

PYTHON_BIN="${VNVNC_PYTHON:-.venv/bin/python}"
[ -x "${PYTHON_BIN}" ] || PYTHON_BIN=python3
PYCACHE_DIR="${ARTIFACT_PYCACHE_DIR:-/tmp/artifact-activate-spiderverse-pycache}"
rm -rf "${PYCACHE_DIR}"
mkdir -p "${PYCACHE_DIR}"
PYTHONPATH=src PYTHONPYCACHEPREFIX="${PYCACHE_DIR}" "${PYTHON_BIN}" -m py_compile \
    src/artifact/ai/caricature.py src/artifact/animation/idle_scenes.py \
    src/artifact/modes/photobooth.py src/artifact/modes/photobooth_themes.py \
    src/artifact/modes/spiderverse_quest.py \
    src/artifact/printing/spiderverse_quest_roll.py
rm -rf "${PYCACHE_DIR}"

echo "SPIDERVERSE photobooth env is active in ${ENV_FILE}"
if [ "${RESTART}" = "1" ]; then
    ARTIFACT_MARK_RESTART_PENDING=1 "${REPO_DIR}/scripts/restart-artifact-if-idle.sh" || true
fi
