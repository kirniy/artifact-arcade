#!/bin/bash
# Activate the VNVNC 9-year birthday photobooth package.
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Activates the VNVNC B'DAY photobooth theme."
            exit 0
            ;;
...
set_env PHOTOBOOTH_THEME vnvnc-bday
# "classic" expands the current theme's three canonical B'DAY variants.
set_env PHOTOBOOTH_MENU_MODES classic
...
PYCACHE_DIR="${ARTIFACT_PYCACHE_DIR:-/tmp/artifact-activate-vnvnc-bday-pycache}"
rm -rf "${PYCACHE_DIR}"
mkdir -p "${PYCACHE_DIR}"
...
echo "VNVNC B'DAY photobooth env is active in ${ENV_FILE}"
if [ "${RESTART}" = "1" ]; then
