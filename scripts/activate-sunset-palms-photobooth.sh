#!/bin/bash
# Activate Sunset Palms only after the accepted fan master passes idle-video QC.
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Activates Sunset Palms after validating the accepted idle fan video."
            exit 0
            ;;
...
# Fail closed before touching .env. The accepted fan master is mandatory.
"${REPO_DIR}/scripts/prepare-sunset-palms-idle-video.sh" --check
...
set_env PHOTOBOOTH_THEME sunset-palms
set_env PHOTOBOOTH_MENU_MODES sunset-palms
...
PYCACHE_DIR="${ARTIFACT_PYCACHE_DIR:-/tmp/artifact-activate-sunset-palms-pycache}"
rm -rf "${PYCACHE_DIR}"
mkdir -p "${PYCACHE_DIR}"
...
echo "Sunset Palms photobooth env is active in ${ENV_FILE}"
if [ "${RESTART}" = "1" ]; then
