#!/bin/bash
# Activate the Cringe Party photobooth configuration. TV wall stays off.
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Sets .env to the Cringe Party three-mode photobooth set. TV wall remains off."
            exit 0
            ;;
        *)
...
set_env PHOTOBOOTH_THEME brainrot
set_env PHOTOBOOTH_MENU_MODES brainrot,wedding,whatsapp
set_env PHOTOBOOTH_CAMERA_SELECTOR_ENABLED false
set_env PHOTOBOOTH_AI_ENABLED false
...
set_env GOOGLE_CLOUD_PROJECT gen-lang-client-0412477988
...
echo "Cringe Party photobooth env is active in ${ENV_FILE}"
